...
For this project it was agreed that it would be best to use a more agile methodology, the primary tasks were agreed but remained flexible in keeping with the customer's desire to ``See what we could do with HTML5 and HLS streams on iPads.'' This choice was also partially because most aspects of the project relied on the correct configuration of the Wowza server which the team had no prior experience of and hence could not easily plan for. In addition the lack of iPads made testing difficult making it hard to estimate exactly what could be done. In addition as we intended to perform several studies to elicit information an agile approach allowed us to mutate our goals and change the requirement based on the outcome of these studies and discussions at our weekly all team meetings. Staying flexible in our approach allowed us to achieve more in the limited period of time available that a more rigid approach which could have led to tasks blocking project progression and a less constructive use of the resources available to us.

Especially when using a more agile approach, such as that which we used in this project, where the exact requirements are less rigid than the overall goals the ability of source control systems to create and merge branches becomes highly desirable. Hence due to our team's past experience with Mercurial and good knowledge of the merging and and branching systems, it was decided that we would use Mercurial as our source control system. In any team project the use of a source control system is vital as it provides security, preventing accidental deletion of work and if a merger does result in some loss of work it can always be restored. In addition as mentioned it provides a highly functional branching system which in the collective team opinion is better than that provided by Subversion and less complex than that provided by git which not all team members had experience with.

We used a free student Mercurial repository hosted by Bitbucket\footnote{Bitbucket: Source Control Provider -- \footurl{https://bitbucket.org/}}. We chose to use Bitbucket rather than hosting our own server as Bitbucket provides enterprise class reliability using RAID6 replication and both off-site and on-site backups\footnote{Bitbucket Reliability: \footurl{https://confluence.atlassian.com/pages/viewpage.action?pageId=288658413}}. This allowed us to get on with the project without worrying about the safety of our code, even if a repository is deleted it is still kept backed up by Bitbucket for fourteen days which would allow us to retrieve our code even in the case of malicious deletion. Finally, Bitbucket repositories include a free JIRA bug tracking plug-in. Again the team had significant experience with using the JIRA system and we were able to use this to keep track of issues and extensions that we were working on which allowed us to be certain we were not forgetting an outstanding issue.
