\documentclass[12pt,a4paper,titlepage]{article}
\usepackage[left=35mm, right=24mm, top=24mm, bottom=24mm]{geometry}
\usepackage{parskip} % http://www.tex.ac.uk/cgi-bin/texfaq2html?label=parskip
\setlength{\parindent}{0cm}
\setlength{\parskip}{0.4cm}


% Clickable references, glossary, footnotes etc.
\usepackage[colorlinks=false,pdfborder={0 0 0}]{hyperref}
\usepackage[flushmargin,hang,multiple,ragged]{footmisc} % nicer footnotes - ragged right

% Bibliography
\usepackage[authoryear,sort&compress]{natbib}
\bibliographystyle{apalike}
%\bibliographystyle{plainnat}
%\bibliographystyle{ieeetr}
%\usepackage{bibunits}
%\usepackage{bibentry}
\renewcommand{\bibsection} % Make bibtex not print a heading for the references and appendix.

% Blue Colored URLs
\usepackage[usenames]{color}
\definecolor{urlcolor}{rgb}{0,0.08,0.45}

% You can footnote URLs with this utility command.
\usepackage{url}
\usepackage{breakurl}
\usepackage[normalem]{ulem}
\newcommand{\link}[2]{{#1}\footnote{{#1} - \footurl{#2} }}
\newcommand{\footurl}[1]{\color{urlcolor}{\textit{\url{#1}}}\color{black}}

% Cool figure captions. Must go before subfig due to clash.
\usepackage[font=small,format=plain,labelfont=bf,up,textfont=it,up]{caption} 

% Fancy headers
\usepackage{fancyhdr}
% \setlength{\headheight}{15.2pt}
\pagestyle{fancy}
\headheight 35pt
%\lhead{Your4.tv}

% Fancy titles
\usepackage{bold-extra}
\usepackage[sc,bf,compact,small]{titlesec}

% Allow use of paragraph as depth 4 sub title
\setcounter{secnumdepth}{4}
\titleformat{\paragraph}{\normalfont\normalsize\bfseries}{\theparagraph}{1em}{}
\titlespacing*{\paragraph}{0pt}{3.25ex plus 1ex minus .2ex}{1.5ex plus .2ex}

% Pretty listings
\usepackage{listings}
\input{PrettyListings}
% \lstset{basicstyle=\scriptsize}

% Allow block comments within \begin{comment} ... \end{comment}
\usepackage{comment}

% Nice appendices
\usepackage[titletoc,title]{appendix}

% Nicer looking table rules.
\usepackage{booktabs}

% Nice pseudocode
\usepackage{algorithmicx}
\usepackage{algpseudocode}

% Do not allow floats to float into following sections.
\usepackage[section]{placeins}

% More math fonts (used for mathbb)
\usepackage{amsfonts}

% Allow text to wrap around certain figures
\usepackage{wrapfig}

\usepackage{amsmath}
\usepackage{graphicx}
\usepackage{float}
\usepackage{lscape}
\usepackage{subcaption}
\usepackage{tabularx}
\usepackage{multicol}
\usepackage{emptypage}
\usepackage{longtable}
\usepackage{array}

\AtBeginDocument{\renewcommand\cite{\citep}} % Redefine \cite to \citep.

\usepackage{enumitem}
